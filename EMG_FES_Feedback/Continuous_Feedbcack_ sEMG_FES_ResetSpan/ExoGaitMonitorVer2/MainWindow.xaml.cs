    ...
    public partial class MainWindow : Window
    {
        ...
        private void ElecFeedback()
        {
            ...
            while (main_s)
            {
                ...
                int PulseCurrentF = 7;
                int PulseFreF = 25;
                double AngelMax = 70.0;

                if (Math.Abs(AngVel[0]) >= AngelMax)
                    AngVel[0] = AngelMax;
                int PulseWidthFL = (int)(AngVel[0]/ AngelMax) * PulseWidthF;
                

                if (Math.Abs(AngVel[3]) >= AngelMax)
                    AngVel[3] = AngelMax;
                int PulseWidthFR = (int)(AngVel[3] / AngelMax) * PulseWidthF;
                ...
                //SingleChannel(1, PulseWidthF, PulseCurrentFR, PulseFreF);




                #region discreteFeedback
        ...
        public void TwoChannel(int PulseWidthL, int PulseWidthR, int PulseCurrent, int  PulseFreq)  //调用单通道函数
        {
            //stimlator.OpenSetup_serial();
            //stimlator.Send_Init_Param(2, 0xff, 0x00, 50, 20, 0);
            while (StimNum <= 1 *  PulseFreq)  //执行1秒钟退出
            {
                stimlator.Send_Single_Pulse( 0,  PulseWidthL,  PulseCurrent);  //channel，Pulse_Width, Pulse_Current
                stimlator.Send_Single_Pulse( 1,  PulseWidthR,  PulseCurrent);
                StimNum++;
                Thread.Sleep( 1000/PulseFreq );
            }
            StimNum = 0;
