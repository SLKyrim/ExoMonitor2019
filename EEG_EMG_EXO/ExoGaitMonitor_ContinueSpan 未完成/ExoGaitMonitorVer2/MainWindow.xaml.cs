    ...
    public partial class MainWindow : Window
    {
      ...
      // 测试用
        ...
        private void control()
        {
            ...
            {
                // 起坐步态
                if (state == 0 && emg_cm == 1 && pattern == 0)  //emg_cm=1，且为坐立状态 时起立
                {
                    pattern = 1; //由坐下到直立
                }
                ...
                //若干正常步（起始步迈左腿 -> 接起始步的正常步迈右腿 ->【正常步迈左腿 ->接正常步的正常步迈右腿】） 【】内为可循环部分
                if (state == 1 && eeg_cm == ENABLE && pattern == 0 && emg_cm == 0) 
                {
                    pattern = 2; //正常步，起始步迈左腿，由直立状态到左腿在前的站姿
                }
                if (state == 2 && eeg_cm == ENABLE && pattern == 0 && emg_cm == 0 )
                {
                    pattern = 3; //正常步，接起始步的正常步迈右腿，由左腿在前的站姿到右腿在前的站姿
                }
                if (state == 3 && eeg_cm == ENABLE && pattern == 0 && emg_cm == 0)
                {
                    pattern = 4; //正常步，正常步迈左腿，由右腿在前的站姿到左腿在前的站姿
                }
                if (state == 4 && eeg_cm == ENABLE && pattern == 0 && emg_cm == 0)
                {
                    pattern = 5; //正常步，接正常步的正常步迈右腿，由左腿在前的站姿到右腿在前的站姿
                }
                if (state == 5 && pattern == 0 && eeg_cm == ENABLE && emg_cm == 0)
                {
                ...
                //    pattern = 5; //正常步，接正常步的正常步迈右腿，由左腿在前的站姿到右腿在前的站姿
                //}
                //if (state == 5 && pattern == 0 && eeg_cm == ENABLE)
                //{
                //    pattern = 4; //正常步迈左腿，由右腿在前的站姿到左腿在前站姿
                //}
                #endregion
                ...
                if (state == 5 && eeg_cm == ENABLE && pattern == 0 && emg_cm == 0)
                {
        ...
        private void SStand_up_Button_Click(object sender, RoutedEventArgs e)//点击【Stand up】按钮时执行:由坐下姿势起立
        {
            Button bt = sender as Button;
            double positon = motors.ampObjAngleActual[3];
            if (bt.Content.ToString() == "Stand Up2")
            {
                PVT_Button.IsEnabled = false;
                angleSetButton.IsEnabled = false;
                Sit_button.IsEnabled = false;
                Stand_up_Button.IsEnabled = false;
                SStand_up_Button.IsEnabled = true;
                statusBar.Background = new SolidColorBrush(Color.FromArgb(255, 230, 20, 20));
                statusInfoTextBlock.Text = "起立模式";
                bt.Content = "Stop";
                if (Math.Abs(positon) > 60)
